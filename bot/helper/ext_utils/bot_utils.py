    ...
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [
                ...
                msg += f"\n<b>📊Progress:</b> {get_progress_bar_string(download)} {download.progress()}"
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>♻️Cloned:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                ...
                try:
                    msg += f"\n<b>⚓️Seeders:</b> {download.aria_download().num_seeders}" \
                           f" | <b>Peers:</b> {download.aria_download().connections}"
                except:
                    pass
                try:
                    msg += f"\n<b>⚓️Seeders:</b> {download.torrent_info().num_seeds}" \
                ...
                msg += f"\n<b>✴️Ratio: </b>{round(download.torrent_info().ratio, 3)}"
