...
def _getResult(search_results, key, message, method):
    ...
    for index, result in enumerate(search_results, start=1):
        if method.startswith('api'):
            ...
            if 'torrents' in result.keys():
                ...
                try:
                    msg += f"<b>⚓️Seeders: </b>{result['seeders']} | <b>Leechers: </b>{result['leechers']}<br>"
                except:
                    pass
            ...
            msg += f"<b>⚓️Seeders: </b>{result.nbSeeders} | <b>Leechers: </b>{result.nbLeechers}<br>"
            link = result.fileUrl
    ...
    path = [telegraph.create_page(
                title='🔍Mirror Bot Torrent Search',
                content=content
